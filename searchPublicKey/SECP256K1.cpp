...
{
	// std::cout<< std::endl << "---- privKey : secp.cpp " <<privKey ;
	// std::cout<< std::endl << "---- &privKey : secp.cpp " <<&privKey ;
	...
	Q = GTable[256 * i + (b - 1)];
	i++;
	// std::cout<<std::endl<<"---- Q 0: \n"<<Q.toString(); // p này có dạng (x=,y=,z=)
...
Int Secp256K1::DecodePrivateKey(char* key, bool* compressed)
{
	...
	if (key[0] == '5') {
		...
		if (c[0] != privKey[33] || c[1] != privKey[34] ||
			...
			printf("Warning, Invalid private key checksum ! ------ 111111111111\n");  
		}

						// print hiiu
						// std::cout<<std::endl<<"\n priv.data() : "<<privKey.data();
						// printf("\n--- : %s \n", privKey.data());

						printf("\n---c[0] : %d	---privKey[33] : %d ", c[0], privKey[33]);
						printf("\n---c[1] : %d	---privKey[34] : %d ", c[1], privKey[34]);
						printf("\n---c[2] : %d	---privKey[35] : %d ", c[2], privKey[35]);
						printf("\n---c[3] : %d	---privKey[36] : %d \n", c[3], privKey[36]);
						// std::cout<<std::endl<<"ret.16 : "<< ret.GetBase16();
						// std::cout<<std::endl<<"ret.10 : "<< ret.GetBase10();
						std::cout<<std::endl<<"end _1_ =======================================";
		...
		*compressed = false;
		...
		if (c[0] != privKey[34] || c[1] != privKey[35] ||
			...
			printf("Warning, Invalid private key checksum ! ---------- 222222222222222\n");
		}
						...
						printf("\n---c[0] : %d	---privKey[34] : %d ", c[0], privKey[34]);
						printf("\n---c[1] : %d	---privKey[35] : %d ", c[1], privKey[35]);
						printf("\n---c[2] : %d	---privKey[36] : %d ", c[2], privKey[36]);
						printf("\n---c[3] : %d	---privKey[37] : %d \n", c[3], privKey[37]);
						// std::cout<<std::endl<<"ret.16 : "<< ret.GetBase16();
						// std::cout<<std::endl<<"ret.10 : "<< ret.GetBase10();
						std::cout<<std::endl<<"end _2_ =============================================================================================";
						std::cout<<std::endl<<std::endl;
