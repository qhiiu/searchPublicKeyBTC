...
{
    // std::cout<<std::endl << "==========  psz base58.cpp = addr : "<<psz ;
    ...
    while (*psz == '1') {
        ...
        // std::cout<<std::endl<<"-------zeroes --base58.cpp : "<<zeroes;
        // std::cout<<std::endl<<"-------psz----base58.cpp : "<<psz<<std::endl;
    }
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (psz[i] & 0x80)
            // std::cout<<std::endl<<"--------test psz[i] : "<<psz[i];
            return false;

        int8_t c = b58digits_map[psz[i]];
        // std::cout<<std::endl<<"--------test c = b57digits_map[psz[i]] : "<<c;

        if (c < 0)
            // std::cout<<std::endl<<"--------test // : "<<psz[i];
            return false;
    ...
    for (int i = 0; i < digitslen; i++)
        vch.push_back(digits[digitslen - 1 - i]);
                ...
                // //print hiiu
            	// std::cout <<std::endl << "------ vch.size() base58.cpp 93 : "<< vch.size() ;
                // for (char x : vch) { 
                //     printf("\n========= vch base58.cpp : %hhx",x);
                // }
                // std::cout << std::endl;
